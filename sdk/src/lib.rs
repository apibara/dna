...
impl ClientBuilder {
    ...
    /// connecting.
    pub async fn connect(self, url: Uri) -> Result<StreamClient> {
...
impl StreamClient {
    ...
    /// If a configuration was provided, the client will immediately send it to the server upon
    /// connecting.
    pub async fn start_stream<F, D, C>(mut self, configuration: C) -> Result<DataStream<F, D, C>>
    where
        F: Message + Default,
        D: Message + Default,
        C: Stream<Item = Configuration<F>> + Send + Sync + 'static,
...
where
    ...
    C: Stream<Item = Configuration<F>> + Send + Sync + 'static,
{
    type Item = Result<DataMessage<D>>;
...
impl MetadataInterceptor {
    fn new(metadata: MetadataMap, token: Option<String>) -> Result<Self> {
