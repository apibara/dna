...
use crate::proto::{
    ExtensionNumberResponse, ExtensionRequest, FileDescriptorResponse, ListServiceResponse,
...
impl ReflectionServiceState {
    ...
    fn file_containing_extension(
        &self,
        _req: &ExtensionRequest,
    ) -> Result<MessageResponse, Status> {
        Ok(MessageResponse::FileDescriptorResponse(
            FileDescriptorResponse {
                file_descriptor_proto: Vec::default(),
            },
        ))
    }

    fn all_extension_numbers_of_type(&self, symbol: &str) -> Result<MessageResponse, Status> {
        Ok(MessageResponse::AllExtensionNumbersResponse(
            ExtensionNumberResponse {
                base_type_name: symbol.to_string(),
                extension_number: Vec::default(),
            },
        ))
    }
}
            ...
            while let Some(req) = req_rx.next().await {
                ...
                let resp_msg = match req.message_request.clone() {
                    ...
                    Some(msg) => match msg {
                        ...
                        MessageRequest::FileContainingExtension(r) => {
                            state.file_containing_extension(&r)
                        }
                        MessageRequest::AllExtensionNumbersOfType(s) => {
                            state.all_extension_numbers_of_type(&s)
                        }
