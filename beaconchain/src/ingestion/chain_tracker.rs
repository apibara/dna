...
use crate::ingestion::{BeaconResponseExt, BlockId};

use super::BeaconApiProvider;
...
async fn track_chain(
    ...
    let mut head = provider
        ...
        .attach_printable("failed to fetch current head")?
        .cursor();
    ...
    let mut finalized = provider
        ...
        .await
        .change_context(DnaError::Fatal)
        .attach_printable("failed to fetch current finalized")?
        .cursor();
