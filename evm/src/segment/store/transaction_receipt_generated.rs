...
impl<'a> TransactionReceipt<'a> {
  ...
  pub fn create<'bldr: 'args, 'args: 'mut_bldr, 'mut_bldr>(
    ...
    if let Some(x) = args.blob_gas_price { builder.add_blob_gas_price(x); }
    if let Some(x) = args.blob_gas_used { builder.add_blob_gas_used(x); }
    if let Some(x) = args.logs_bloom { builder.add_logs_bloom(x); }
    if let Some(x) = args.contract_address { builder.add_contract_address(x); }
    if let Some(x) = args.to { builder.add_to(x); }
    if let Some(x) = args.from { builder.add_from(x); }
    if let Some(x) = args.effective_gas_price { builder.add_effective_gas_price(x); }
    if let Some(x) = args.gas_used { builder.add_gas_used(x); }
    if let Some(x) = args.cumulative_gas_used { builder.add_cumulative_gas_used(x); }
  ...
  pub fn transaction_hash(&self) -> Option<&'a B256> {
    ...
    unsafe { self._tab.get::<B256>(TransactionReceipt::VT_TRANSACTION_HASH, None)}
  }
  #[inline]
  pub fn transaction_index(&self) -> u64 {
    ...
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<u64>(TransactionReceipt::VT_TRANSACTION_INDEX, Some(0)).unwrap()}
  }
  #[inline]
  pub fn cumulative_gas_used(&self) -> Option<&'a U256> {
    ...
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<U256>(TransactionReceipt::VT_CUMULATIVE_GAS_USED, None)}
  }
  #[inline]
  ...
  pub fn effective_gas_price(&self) -> Option<&'a U128> {
    ...
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<U128>(TransactionReceipt::VT_EFFECTIVE_GAS_PRICE, None)}
  }
  #[inline]
  ...
  pub fn contract_address(&self) -> Option<&'a Address> {
    ...
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<Address>(TransactionReceipt::VT_CONTRACT_ADDRESS, None)}
  }
  #[inline]
  pub fn logs_bloom(&self) -> Option<&'a Bloom> {
    ...
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<Bloom>(TransactionReceipt::VT_LOGS_BLOOM, None)}
  }
  #[inline]
  pub fn status_code(&self) -> u64 {
    ...
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<u64>(TransactionReceipt::VT_STATUS_CODE, Some(0)).unwrap()}
  }
  #[inline]
  pub fn transaction_type(&self) -> u64 {
    ...
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<u64>(TransactionReceipt::VT_TRANSACTION_TYPE, Some(0)).unwrap()}
  }
  #[inline]
  pub fn blob_gas_used(&self) -> Option<&'a U128> {
    ...
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<U128>(TransactionReceipt::VT_BLOB_GAS_USED, None)}
  }
  #[inline]
  pub fn blob_gas_price(&self) -> Option<&'a U128> {
    ...
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<U128>(TransactionReceipt::VT_BLOB_GAS_PRICE, None)}
  }
}
...
impl<'a: 'b, 'b> TransactionReceiptBuilder<'a, 'b> {
  ...
  pub fn add_transaction_hash(&mut self, transaction_hash: &B256) {
    self.fbb_.push_slot_always::<&B256>(TransactionReceipt::VT_TRANSACTION_HASH, transaction_hash);
  }
  #[inline]
  pub fn add_transaction_index(&mut self, transaction_index: u64) {
    self.fbb_.push_slot::<u64>(TransactionReceipt::VT_TRANSACTION_INDEX, transaction_index, 0);
  }
  #[inline]
  pub fn add_cumulative_gas_used(&mut self, cumulative_gas_used: &U256) {
    self.fbb_.push_slot_always::<&U256>(TransactionReceipt::VT_CUMULATIVE_GAS_USED, cumulative_gas_used);
  }
  #[inline]
  pub fn add_gas_used(&mut self, gas_used: &U256) {
    self.fbb_.push_slot_always::<&U256>(TransactionReceipt::VT_GAS_USED, gas_used);
  }
  #[inline]
  pub fn add_effective_gas_price(&mut self, effective_gas_price: &U128) {
    self.fbb_.push_slot_always::<&U128>(TransactionReceipt::VT_EFFECTIVE_GAS_PRICE, effective_gas_price);
  }
  #[inline]
  pub fn add_from(&mut self, from: &Address) {
    self.fbb_.push_slot_always::<&Address>(TransactionReceipt::VT_FROM, from);
  }
  #[inline]
  pub fn add_to(&mut self, to: &Address) {
    self.fbb_.push_slot_always::<&Address>(TransactionReceipt::VT_TO, to);
  }
  #[inline]
  pub fn add_contract_address(&mut self, contract_address: &Address) {
    self.fbb_.push_slot_always::<&Address>(TransactionReceipt::VT_CONTRACT_ADDRESS, contract_address);
  }
  #[inline]
  pub fn add_logs_bloom(&mut self, logs_bloom: &Bloom) {
    self.fbb_.push_slot_always::<&Bloom>(TransactionReceipt::VT_LOGS_BLOOM, logs_bloom);
  }
  #[inline]
  pub fn add_status_code(&mut self, status_code: u64) {
    self.fbb_.push_slot::<u64>(TransactionReceipt::VT_STATUS_CODE, status_code, 0);
  }
  #[inline]
  pub fn add_transaction_type(&mut self, transaction_type: u64) {
    self.fbb_.push_slot::<u64>(TransactionReceipt::VT_TRANSACTION_TYPE, transaction_type, 0);
  }
  #[inline]
  pub fn add_blob_gas_used(&mut self, blob_gas_used: &U128) {
    self.fbb_.push_slot_always::<&U128>(TransactionReceipt::VT_BLOB_GAS_USED, blob_gas_used);
  }
  #[inline]
  pub fn add_blob_gas_price(&mut self, blob_gas_price: &U128) {
    self.fbb_.push_slot_always::<&U128>(TransactionReceipt::VT_BLOB_GAS_PRICE, blob_gas_price);
  }
  #[inline]
  pub fn new(_fbb: &'b mut flatbuffers::FlatBufferBuilder<'a>) -> TransactionReceiptBuilder<'a, 'b> {
