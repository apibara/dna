...
pub struct SegmentFilter<S: StorageBackend + Send> {
    ...
    event_segment_reader: LazySegmentReader<S, SegmentDataOptions, store::EventSegment>,
    message_segment_reader: LazySegmentReader<S, SegmentDataOptions, store::MessageSegment>,
    transaction_segment_reader: LazySegmentReader<S, SegmentDataOptions, store::TransactionSegment>,
    receipt_segment_reader:
        LazySegmentReader<S, SegmentDataOptions, store::TransactionReceiptSegment>,
}
    ...
    S::Reader: Unpin + Send,
{
    ...
    pub async fn filter_segment_block(
        ...
        if should_load_messages {
            ...
            for (filter, block_data) in self.filters.iter().zip(blocks_data.iter_mut()) {
                for message in messages.data.iter() {
