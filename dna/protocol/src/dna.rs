                ...
                A: serde::de::MapAccess<'de>,
            {
                ...
                while let Some(key) = map.next_key()? {
                    match key {
                        ...
                        field => {
                            return Err(de::Error::unknown_field(field, &["orderKey", "uniqueKey"]))
                        }
                    }
                }
                let order_key = order_key.ok_or_else(|| de::Error::missing_field("orderKey"))?;
                let unique_key = unique_key.ok_or_else(|| de::Error::missing_field("uniqueKey"))?;
                Ok(Cursor {
                    order_key,
                    unique_key,
                })
...
#[cfg(test)]
mod tests {
    use crate::dna::{Cursor, DataFinality};
