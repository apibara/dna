...
use super::partition::{FlushError, PartitionFolio, PartitionFolioWriter};
...
#[derive(Debug)]
pub struct PartitionFolioWithMetadata {
    pub folio: PartitionFolio,
    pub topic: TopicName,
...
impl NamespaceFolioWriter {
    ...
    ) -> std::result::Result<Option<FlushedFolio>, (WriteReplySender, Report<IngestorError>)> {
...
impl NamespaceFolioState {
    ...
    pub fn finish(self) -> FlushedFolio {
        ...
        for (key, partition_writer) in self.partitions.into_iter() {
            ...
            match partition_writer.finish() {
                Ok(folio) => partitions.push(PartitionFolioWithMetadata {
                    folio,
                    topic: topic_name,
                    partition,
                }),
...
impl Default for NamespaceFolioWriter {
    fn default() -> Self {
        Self {
...
impl FlushedFolio {
    pub fn reply_with_errors_and_continue(self) -> FolioToUpload {
        for reply in self.errors {
            reply.send_reply();
        }
